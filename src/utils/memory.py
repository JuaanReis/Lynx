...
def check_memory_limit(limit_mb):
    ...
    if memory_usage > limit_mb:
        print(f"{Fore.WHITE}[!]{Fore.WHITE} Uso de memória excedeu o limite de {limit_mb}MB. Usando {memory_usage:.2f}MB. Abortando...\n")
        sys.exit(1)  # Finaliza o programa, você pode escolher outra ação como liberar memória ou algo mais
    else:
        print(f"{Fore.GREEN}[+]{Fore.WHITE} Memória usada: {memory_usage:.2f}MB (dentro do limite)\n")
